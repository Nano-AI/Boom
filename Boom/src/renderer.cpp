...
bool Renderer::isDeviceSuitable(VkPhysicalDevice device) {
	...
	//For now, populate a queue family struct to check if the device can process the right commands
...
void Renderer::pickPhysicalDevice() {
	// Selected graphics card stored here; implicitly destroyed when VkInstance is destroyed so don't update cleanup()
	physicalDevice = VK_NULL_HANDLE;

	// If theres no GPUs found then throw an error (duh)
	uint32_t deviceCount;
	vkEnumeratePhysicalDevices(instance, &deviceCount, nullptr);

	bool deviceSuccess = (deviceCount != 0);
	...
	bool physicalHandleSuccess = (physicalDevice != VK_NULL_HANDLE);

	if (!physicalHandleSuccess) {
		LOG_F(ERROR, "Failed to find a suitable GPU!");
	}


	//UNCOMMENT for a score/point based system for determining the best available physical device
